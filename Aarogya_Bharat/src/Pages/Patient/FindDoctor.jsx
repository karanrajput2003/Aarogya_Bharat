...
// Example images, replace these with actual images for each doctor.
      ...
      <section className="w-full min-h-screen pt-6 md:pt-12 lg:pt-16 bg-gradient-to-b from-[#073243] via-[#0a4c59] to-[#0d6270]">
        <div className="container mx-auto space-y-10 xl:space-y-16 px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex justify-center mt-6">
            <label htmlFor="specialization" className="text-white font-medium mr-4">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filteredDoctors.map((doctor) => (
              <div key={doctor.id} className="bg-white rounded-lg p-6 shadow-md hover:shadow-xl transition-shadow">
                <div className="flex flex-col items-center">
                  <img
                    src={doctor.image}
                    alt={doctor.name}
                    className="w-32 h-32 rounded-full border-4 border-[#0d6270]"
                  />
                  ...
                  <p className="text-gray-500 text-sm mt-1">{doctor.reviews.length} reviews</p>
                  <Link to={`/patient/doctor/${doctor.id}`} className="mt-4 bg-[#0d6270] text-white py-2 px-4 rounded hover:bg-[#0a4c59] transition">
                    Book Appointment
                  </Link>
                </div>
              </div>
